...
export class MakeOrderService {
    ...
    public async invoke(createActivityRequest: IMakeOrderRequest): Promise<void> {
        await this.orderRepository.makeOrder(createActivityRequest.userId,createActivityRequest.total_amount, createActivityRequest.status, createActivityRequest.order_items)
    }
}